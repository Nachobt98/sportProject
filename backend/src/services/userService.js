...
const { getProfileImagePublicPath, removeLocalProfileImage } = require("../utils/profileImages");

const editableUserFields = ["firstName", "lastName", "city", "email", "birthdate"];

function buildEditableUserPayload(payload) {
  if (payload.profileImage !== undefined) {
    return { error: "La imagen de perfil debe actualizarse desde el endpoint de subida", code: ERROR_CODES.VALIDATION_ERROR };
  }
...
async function getCurrentUser(userName) {
  ...
  if (!user) return serviceResponse(404, "Usuario no encontrado", {}, ERROR_CODES.USER_NOT_FOUND);
...
async function updateCurrentUser(userName, payload) {
  ...
  if (error) return serviceResponse(400, error, {}, code);
  ...
  if (value.email && value.email !== user.email) {
    ...
    if (duplicatedEmail) return serviceResponse(409, "Ya existe un usuario con ese email", {}, ERROR_CODES.DUPLICATE_EMAIL);
  }
...
async function updateCurrentUserProfileImage(userName, file) {
  if (!file) return serviceResponse(400, "La imagen de perfil es requerida", {}, ERROR_CODES.VALIDATION_ERROR);
